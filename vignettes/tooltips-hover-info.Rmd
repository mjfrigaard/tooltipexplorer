...
A **UI helper** (in `mod_tooltip.R`) — returns a `shiny.tag` with no server-side counterpart. Place it anywhere inside a UI tree, including inside `renderUI()`.
...
A **rendering helper** (in `mod_hoverinfo.R`) — formats hover content for back-ends that build tooltips programmatically inside table-cell renderers.
