...
`tooltipexplorer` ships a dark, Bloomberg-terminal aesthetic: a near-black background, amber primary accent, green / red / cyan data colors, and IBM Plex Mono typography throughout. The look is produced by three cooperating pieces.
...
-   `mod_outputs_server()` gives the KPI value boxes a custom `bslib::value_box_theme(bg = "#ff9e1b", fg = "#05070a")` (amber / near-black) for Sharpe ratios `>= 1`, in place of the Bootstrap `"success"` (green) theme, so the strongest performers read as amber-on-black to match the primary accent. `"warning"` and `"danger"` are used unchanged for the middle and low Sharpe tiers.
