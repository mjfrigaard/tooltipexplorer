...
Each module is a pair of functions, `mod_<name>_ui()` and
`mod_<name>_server()`, co-located in a single `R/mod_<name>.R` file. The
UI function returns a Shiny tag object; the server function wraps its
logic in
...
There is no persistent object between the UI call and the server call —
the pairing is a naming convention, not an enforced binding.
...
Downstream modules receive upstream reactives as ordinary function
arguments:
...
Each reactive depends only on the one before it, so the pipeline can be
reasoned about (and tested) one stage at a time.

### 5. Logging with Namespace Isolation

Every `logger::log_*()` call carries an explicit `namespace` string
scoped to its module:
