...
Typography is IBM Plex Mono throughout (headings, body, and code),
giving the app a fixed-width terminal feel.
...
sidebar, scrollbars, a `.rt-*` reactable fallback, and the third-party
widgets that don’t inherit `bslib` theme variables automatically —
`selectize` (ticker picker), `ionRangeSlider` (rolling-vol window),
...
`template: theme` uses a dark syntax-highlighting theme so code blocks
read against the same near-black background as the rest of the site.
